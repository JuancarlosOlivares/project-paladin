...
class PeripheralTableViewController: UITableViewController, CBCentralManagerDelegate {
    ...
    var storedPeripheral:BlePeripheral?
    ...
    func startBleScan() {
        ...
        scanTimer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(PeripheralTableViewController.updateScanCounter), userInfo: nil, repeats: true)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let p = storedPeripheral
        if(p == nil)
        {
            //let tableViewController = segue.destination as! PeripheralTableViewController
            ...
            if let selectedIndexPath = tableView.indexPathForSelectedRow {
                ...
                if selectedRow < blePeripherals.count {
                    // prepare next UIView
                    storedPeripheral = blePeripherals[selectedRow]
                    
                    //storedPeripheral = setIsTable2(value: true)
                }
            ...
            print("ABOUT TO SEGUE WITH FOLLOWING SELECTED: " + blePeripherals[selectedRow].advertisedName)
            performSegue(withIdentifier: "vcSegue", sender: nil)
        }
        print("SELECTED: " + blePeripherals[selectedRow].advertisedName)
    }
    
    func setstoredPeripheral(peripheral: BlePeripheral){
        ...
        if(segue.identifier == "vcSegue") {
                ...
                if let selectedIndexPath = tableView.indexPathForSelectedRow {
                    ...
                    if selectedRow < blePeripherals.count {
                        ...
                        viewController.loPeriph = blePeripherals[selectedRow]
                        viewController.vuPeriph = p!
                    }
                    tableView.deselectRow(at: selectedIndexPath, animated: true)
                }
            }
